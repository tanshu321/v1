...
function AddInvoice()
{
	...
	if (Error == 0)
	{
		...
		{
			var AddInvoiceHeader = agent.call('','AddInvoiceHeader','', UseInvoiceReference, CustomerReference, DueDate, VATNumber, DiscountPercent, Address1, Address2, City, State, PostCode, Country, '<?php echo $CustomerID ?>', '<?php echo $TaxExempt ?>', InvoiceNotes, '<?php echo $JobID ?>');
			...
			if (parseInt(AddInvoiceHeader) > 0)
			{
				...
				document.location = 'editcustomerinvoice.php?c=<?php echo $CustomerID ?>&i=' + AddInvoiceHeader;
			}
			else
			{
                                        ...
                                        <h4 style="padding-bottom: 10px">Invoice Details</h4>
                                                ...
                                                <!-- END FORM CONTROLS -->
                                                 <div class="form-group row col-md-12" align="right" style="padding-top: 30px; padding-right: 120px">
                                                	<button class="btn btn-info" onClick="javascript: AddInvoice();" style="">Next >></button> 
