...
function GetSingleClient($CustomerID)
{
	...
	$GetClient = "SELECT * FROM customers, countries WHERE customers.CountryID = countries.CountryID AND CustomerID = {$CustomerID}";
...
function TotalInvoicesCAR($InvoiceStatus)
{
	...
	customers.CompanyName as CompanyName, SUM(LineTotal) AS InvoiceTotals FROM customerinvoices, customerinvoicelines,
...
function TotalInvoicesByDays($monthName, $CustomerID, $InvoiceStatus,$latMonth=FALSE)
{
	...
	$CountInvoices = "SELECT SUM(LineTotal) AS InvoiceTotals FROM customerinvoices, customerinvoicelines WHERE
...
function AddInvoiceLineProRata($InvoiceID, $ProductID, $Price, $Quantity, $DiscountPercent, $NumBillingDays, $Days)
{
	$ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], 	$_SESSION["DBName"]);
...
function AddProductRecurring($Product, $Price, $Quantity, $InvoiceType, $CustomerID,  $NextRun, $TaxExempt, $Address1, $Address2, $City, $State, $PostCode, $Country, $RecurringTimes, $WareHouseID)
{
	...
	if ($Error == "")
	{
		if ($InvoiceType == 1) //CREATE A PRO RATA INVOICE AND RECUR ON RECURRING DATE
		{
			...
			if ($NewInvoiceID > 0)
			{
				$AddInvoiceItem = AddInvoiceLineProRata($NewInvoiceID, $Product, $Price, $Quantity, 0, $NumBillingDays, $Days);
...
function AddCustomInvoiceItem($InvoiceID, $CustomItem, $CustomItemPrice, $Quantity, $DiscountPercent, $CostPrice)
{
	$ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], 	$_SESSION["DBName"]);
...
function AddCustomQuoteItem($QuoteID, $CustomItem, $CustomItemPrice, $Quantity, $DiscountPercent, $BillingType)
{
	$ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], 	$_SESSION["DBName"]);
...
function AddQuoteLine($QuoteID, $ProductID, $Price, $Quantity, $DiscountPercent)
{
$ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], 	$_SESSION["DBName"]);
...
function EmailCustomerQuote($QuoteID, $CustomerID)
{
	...
	$pdf->addText(350,790,10,"<b>Quote Number</b>"); $pdf->addText(450,790,10,'QR' . $QuoteID);
	$pdf->addText(350,770,10,"<b>Quote Date</b>"); $pdf->addText(450,770,10,$QuoteDate);
	$pdf->addText(350,750,10,"<b>Expiry Date</b>"); $pdf->addText(450,750,10,$ExpiryDate);
	...
	$ok = @mail($EmailAddress, "Quote - QR" . $QuoteID, $message, $headers);
...
function SendCustomerInvoice($CustomerID, $InvoiceID)
{
	...
	//$ok = @mail($EmailAddress, "Invoice - " . $InvoiceNumber, $message, $headers);
...
function ResendSupplierPO($PurchaseID, $SupplierID)
{
	...
	//$ok = @mail($SupplierEmail, "Purchase Order - " . $PurchaseNumber, $message, $headers);
...
function GetCustomerOpeningStatement($FromDate, $ToDate, $CustomerID)
{
	...
	$GetOpening = "SELECT SUM(LineTotal) AS TotalDebits FROM customerinvoices, customerinvoicelines WHERE customerinvoicelines.InvoiceID = customerinvoices.InvoiceID AND customerinvoices.CustomerID = {$CustomerID}  AND InvoiceDate < '{$FromDate}' AND InvoiceStatus IN(1,6)";
...
function GetCustomerStatement($FromDate, $ToDate, $CustomerID)
{
	...
	{
       //FIRST GET INVOICES
	   $GetInvoices = "SELECT SUM(LineTotal) AS TotalDebits, customerinvoices.* FROM customerinvoices, customerinvoicelines WHERE customerinvoicelines.InvoiceID = customerinvoices.InvoiceID
	   AND customerinvoices.CustomerID = {$CustomerID}  AND InvoiceDate = '{$FromDate}' AND InvoiceStatus != 0 AND InvoiceStatus != 3 GROUP BY InvoiceID ORDER BY customerinvoices.InvoiceID ASC";  
...
function SendCustomerStatement($CustomerID, $FromDate, $ToDate)
{
	...
	while ($Val = mysqli_fetch_assoc($GotCustomer))
	{
	...
	$pdf->addText(350,810,9,"<b>Client Statement</b>"); $pdf->addText(450,810,9,$CompanyName);
	$pdf->addText(350,790,9,"<b>From Date</b>"); $pdf->addText(450,790,9,$FromDate);
	$pdf->addText(350,770,9,"<b>To Date</b>"); $pdf->addText(450,770,9,$ToDate);
	...
	$data[] = array('<b>Date</b>'=>'','<b>Reference</b>'=>'', '<b>Description</b>'=>'<b>Totals</b>', '<b>Debit</b>'=>'<b>R' . number_format($DebitTotal,2) . '</b>', '<b>Credit</b>'=>'<b>R' . number_format($CreditTotal,2) . '</b>');
	$data[] = array('<b>Date</b>'=>'','<b>Reference</b>'=>'', '<b>Description</b>'=>'', '<b>Debit</b>'=>'', '<b>Credit</b>'=>'');
	...
	$ok = @mail('alex@allweb.co.za', "Customer Statement", $message, $headers);
...
function GetCustomerStatementReport($FromDate, $ToDate, $CustomerID)
{
	...
	AND InvoiceStatus != 0 GROUP BY InvoiceID";*/
	
	$GetInvoices = "SELECT SUM(LineTotal) AS TotalDebits, customerinvoices.* FROM customerinvoices, customerinvoicelines WHERE customerinvoicelines.InvoiceID = customerinvoices.InvoiceID AND customerinvoices.CustomerID = {$CustomerID}  AND InvoiceDate >= '{$FromDate}' AND InvoiceDate <= '{$ToDate}'
	AND InvoiceStatus IN(1,6) GROUP BY InvoiceID";
