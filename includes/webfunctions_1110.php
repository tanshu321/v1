...
function GetCustomerOpeningStatement($FromDate, $ToDate, $CustomerID)
{
    ...
    echo $GetOpening = "SELECT SUM(LineTotal) AS TotalDebits FROM customerinvoices, customerinvoicelines
...
print("<br />");
    ...
    echo $GetPayments = "SELECT SUM(TotalPayment) AS TotalCredits FROM customertransactions WHERE PaymentDate < '{$FromDate}' AND CustomerID = {$CustomerID}";
    print("<br />");
    $GotPayments = mysqli_query($ClientCon, $GetPayments);
   ...
   // print("<br />");
        ...
        while ($Val = mysqli_fetch_array($GotInvoices)) {
            ...
            if($Val['InvoiceStatus']=='2'){
                $StatmentArray[$X]["Credit"] = $InvoiceAmount;
                $StatmentArray[$X]["Debit"] = $InvoiceAmount;
            }else{
                $StatmentArray[$X]["Debit"] = $InvoiceAmount;
                $StatmentArray[$X]["Credit"] ="";
...
function SendCustomerStatement($CustomerID, $FromDate, $ToDate)
{
    ...
    $AccountBalance = $DebitTotal - $allCreaditTotal;
    if ($AccountBalance > 0) {
