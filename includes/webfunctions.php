...
function updateCreditTransaction($CustomerID, $InvoiceID, $InvoiceTotal, $creadit_amount)
{
...
function getSalesProducts($FromDate="",$ToDate="",$productname="",$productgroup=""){
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    ...
    if($productgroup!=''){
        ...
        $sql = "select products.ProductID as ProductID, products.ProductName as ProductName, products.ProductGroupID as ProductGroupID, products.ProductSubGroupID as ProductSubGroupID,products.ProductStatus as ProductStatus
    ...
    $DoQuery = mysqli_query($ClientCon, $sql);
...
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    $CreateTable_Query = "CREATE TABLE IF NOT EXISTS `customerproject` ( `ProjectId` INT NOT NULL AUTO_INCREMENT, `CustomerID` INT NOT NULL , `ProjectName` VARCHAR(255) NOT NULL , `StartDate` DATE NOT NULL , `EndDate` DATE NOT NULL , `CreatedDate` DATETIME NOT NULL , PRIMARY KEY (`ProjectId`)) ENGINE = InnoDB;";
    $createInvoiceLineTableQuery = "CREATE TABLE IF NOT EXISTS `customerprojectinvoicelines` ( `ProjectInvoiceLineItemID` int(11) NOT NULL AUTO_INCREMENT, `ProjectID` int(11) NOT NULL, `InvoiceID` int(11) NOT NULL, `Description` varchar(255) DEFAULT NULL, `Quantity` double DEFAULT NULL, `Price` double DEFAULT NULL, `ProductID` int(11) DEFAULT NULL, `ProductCode` varchar(255) DEFAULT NULL, `LineSubTotal` double DEFAULT NULL, `LineDiscount` double DEFAULT NULL, `LineVAT` double DEFAULT NULL, `LineTotal` double DEFAULT NULL, `BillingType` varchar(255) DEFAULT NULL, `StockAffect` int(11) DEFAULT NULL, `MeassurementDescription` varchar(255) DEFAULT NULL, `UnitPrice` double NOT NULL DEFAULT '0', `Profit` double NOT NULL DEFAULT '0', `UnitPriceCost` double NOT NULL DEFAULT '0', `WarehouseID` int(11) NOT NULL DEFAULT '1', PRIMARY KEY (`ProjectInvoiceLineItemID`) )ENGINE = InnoDB";
    $cretaeInvoiceTableQry = "CREATE TABLE IF NOT EXISTS  `customerprojectinovice` ( `ProjectInvoiceId` INT NOT NULL AUTO_INCREMENT , `ProjectId` INT NOT NULL , `InvoiceId` INT NOT NULL , PRIMARY KEY (`ProjectInvoiceId`)) ENGINE = InnoDB;";
    $CreateTable = mysqli_query($ClientCon, $CreateTable_Query);
    $CreateInvoiceLineTable = mysqli_query($ClientCon, $createInvoiceLineTableQuery);
    $CreateInvoiceTable = mysqli_query($ClientCon, $cretaeInvoiceTableQry);
    echo mysqli_error($ClientCon);
    $GetProjects = "SELECT * FROM customerproject WHERE CustomerID = {$CustomerID}";
    $GotProjects = mysqli_query($ClientCon, $GetProjects);
    echo mysqli_error($ClientCon);
...
function GetSingleProject($ProjectId)
{
    ...
    $GetProject = "SELECT * FROM customerproject WHERE ProjectId = {$ProjectId}";
    $GotProject = mysqli_query($ClientCon, $GetProject);
...
function GetAllCustomerInvoicesForProject($CustomerID,$ProjectID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    $ThisClientID = $_SESSION["ClientID"];

    $GetInvoices = "SELECT * FROM customerinvoices WHERE CustomerID = {$CustomerID} AND InvoiceID NOT IN (SELECT InvoiceId FROM customerprojectinovice WHERE ProjectId = {$ProjectID}) ORDER BY InvoiceID DESC";
    $GotInvoices = mysqli_query($ClientCon, $GetInvoices);
    echo mysqli_error($ClientCon);
...
function CreateProject($projectName, $startDate, $endDate, $CustomerID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    $ThisClientID = $_SESSION["ClientID"];
    ...
    $AddProject = "INSERT INTO customerproject (CustomerID, ProjectName, StartDate, EndDate, CreatedDate) ";
    $AddProject .= "VALUES ({$CustomerID}, '{$projectName}', '{$startDate}', '{$endDate}', '{$CreatedDate}')";
    $DoAddProject = mysqli_query($ClientCon, $AddProject);
    ...
    if ($Error == "") {
        ...
        return "There was an error creating this Project, please check your values and try again";
    }
}
...
function AddInvoiceHeader($UseInvoiceReference, $CustomerReference, $DueDate, $VATNumber, $DiscountPercent, $Address1, $Address2, $City, $State, $PostCode, $Country, $CustomerID, $TaxExempt, $InvoiceNotes, $JobID)
{
    ...
    $EmployeeID = isset($Val["EmployeeID"])?$Val["EmployeeID"]:'';
    if ($EmployeeID == "") {
        $EmployeeID = 0;
    }
...
function UpdateProjectQuantity($InvoiceLineItemID,$ProductID,$Quantity,$CustomerID){
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    ...
    //Invoice DETAILS
    $GetInvoiceDetails = "SELECT * FROM customerprojectinvoicelines WHERE ProjectInvoiceLineItemID = {$InvoiceLineItemID}";
    $GotInvoiceDetails = mysqli_query($ClientCon, $GetInvoiceDetails);

    while ($Val = mysqli_fetch_array($GotInvoiceDetails)) {
        $BillingType = $Val["BillingType"];
        $ClientCost = $Val["Price"]; 
        $StockAffect = $Val["StockAffect"];  
        $LineSubTotal = $Val["LineSubTotal"];
        $DiscountAmount = $Val["LineDiscount"];
        $DiscountPercent = ($DiscountAmount*100)/$LineSubTotal;
    }
    ...
    $UPDATELine = "UPDATE customerprojectinvoicelines 
    ...
    UnitPrice = '{$UnitCost}',
    UnitPriceCost = '{$SupplierUnitCost}',
    Profit = '{$LineProfit}'
    WHERE ProjectInvoiceLineItemID = {$InvoiceLineItemID} ";
    ...
    if ($Error == "") {
        ...
        return "There was an error updating quantity";
    }
}
...
function GetCustomerInvoiceLines($ProjectID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);

    $GetLines = "SELECT * FROM customerprojectinvoicelines WHERE ProjectID = {$ProjectID}";
    $GotLines = mysqli_query($ClientCon, $GetLines);
    echo mysqli_error($ClientCon);
    return $GotLines;
}
function GetCustomerJobCards($ProjectID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);

    $GetLines = "SELECT * FROM jobcards WHERE ProjectID = {$ProjectID}";
    $GotLines = mysqli_query($ClientCon, $GetLines);
    echo mysqli_error($ClientCon);
    return $GotLines;
}

function GetCustomerInvoice($ProjectID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);

    $Getinvoice = "SELECT cpv.*,cv.* FROM customerprojectinovice as cpv INNER JOIN customerinvoices as cv ON cv.InvoiceID =cpv.InvoiceId  WHERE cpv.ProjectId = {$ProjectID}";
    $Gotinvoice = mysqli_query($ClientCon, $Getinvoice);
    echo mysqli_error($ClientCon);
    return $Gotinvoice;
}
...
function PublishInvoice($InvoiceID, $PublishType, $CustomerID)
{
    ...
    $UpdateInvoice = "UPDATE customerinvoices SET InvoiceStatus = 1, PublishDate ='{$PublishDate}' WHERE InvoiceID = {$InvoiceID}";
    $DoUpdateInvoice = mysqli_query($ClientCon, $UpdateInvoice);
    ...
    if ($Error == "") {
        ...
        return "There was an error publishing the invoice, please check and try again".$Error ;
    }
}
...
function UpdateProjectStatus($NewStatus, $ProjectId)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);

    $UpdateStatus = "UPDATE customerproject SET status = {$NewStatus} WHERE ProjectId = {$ProjectId}";
    $DoUpdateStatus = mysqli_query($ClientCon, $UpdateStatus);
...
function UpdateProject($projectName, $startDate, $endDate, $CustomerID,$ProjectID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    ...
    $UpdateStatus = "UPDATE customerproject SET projectName = '{$projectName}' , StartDate = '{$startDate}' , EndDate = '{$endDate}' WHERE ProjectId = {$ProjectID}";
    $DoUpdateStatus = mysqli_query($ClientCon, $UpdateStatus);
...
function SendCustomerInvoice($CustomerID, $InvoiceID,$etid = 1)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
...
if($TaxExempt == 0) {
    ...
    $Gettemplate = "SELECT * FROM emailtemplates WHERE id = {$etid}";
    $Gottemplate = mysqli_query($ClientCon, $Gettemplate);
    $emailmessageTemplates="";
    $emailsubject="";
    while ($Val1 = mysqli_fetch_array($Gottemplate)) {
        $emailmessageTemplates=$Val1['content'];
        $emailsubject=$Val1['subject'];
    }

    $signature = 'Kind Regards<br>
                              ' . $DisplayCompany . '<br>
                              ' . $DisplayTel ;
    $emailmessageTemplates = str_replace('#FIRSTNAME#', $Name,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#INVOICENUM#', $InvoiceNumber,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#SURNAME#', $Surname,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#EMAIL#', $EmailAddress,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#SIGNATURE#', $signature,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#COMPANYNAME#', $CompanyName,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#INVOICETOTAL#', number_format($InvoiceTotal, 2),$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#PAYMENTMETHOD#', $PaymentMethod,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#DUEDATE#', $DueDate,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#INVOICEDATE#', $InvoiceDate,$emailmessageTemplates);
    $emailmessageTemplates = str_replace('#CUSTOMERCODE#', $DepositReference,$emailmessageTemplates);
    //EMAIL MESSAGE
    $SupplierMail = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
					<html xmlns="http://www.w3.org/1999/xhtml">
					<head>
    ...
    $SupplierMail = str_replace('#TEST#', $emailmessageTemplates,$SupplierMail);
    $headers = "From: " . $DisplayEmail;
    ...
    mail($EmailAddress, $emailsubject, $message, $headers);
    //UPDATE INVOICE STATUS TO UNPAID
...
function ResendCustomerInvoice($CustomerID, $InvoiceID)
{
    ...
    return SendCustomerInvoice($CustomerID, $InvoiceID,2);
}
/*
function ResendCustomerInvoice($CustomerID, $InvoiceID)
{
...
//CustomerProjects
function GetAllCustomerProjects($Status)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);

    $GetCustomerProjects = "SELECT * FROM customerproject, customers WHERE customerproject.CustomerID = customers.CustomerID ";
    if($Status>0)
        $GetCustomerProjects .= " AND customerproject.status = {$Status}";

    $GetCustomerProjects .= " ORDER BY customerproject.CustomerID DESC";
    $GotCustomerProjects = mysqli_query($ClientCon, $GetCustomerProjects);

    return $GotCustomerProjects;
}
...
function UpdateJobcard($Employee, $ScheduledDate, $Notes, $TechNotes, $InvoiceID,$ProjectID, $JobcardID, $CurrentStatus, $NewJobcardFile, $ManualJobcardNumber,
                       $TotalTime, $Site, $WorkOrder)
{
    ...
    if ($InvoiceID == "") {
        ...
        if ($CurrentStatus == 1) {
            ...
            $UpdateJobcard = "UPDATE jobcards SET AssignedTo = {$Employee}, DateScheduled = '{$ScheduledDate}', JobcardNotes = '{$Notes}', TechReport = '{$TechNotes}', JobcardStatus = {$NewStatus}, InvoiceID = {$InvoiceID},ProjectID = {$ProjectID}, DateInvoice = '{$InvoiceDateAdded}', ManualJobcardNumber = '{$ManualJobcardNumber}', TotalTime = '{$TotalTime}', WorkOrder = '{$WorkOrder}' WHERE JobcardID = {$JobcardID} ";
        } else {
            //JUST AN UPDATE TO ALREADY INVOICED JOB
            $UpdateJobcard = "UPDATE jobcards SET AssignedTo = {$Employee}, DateScheduled = '{$ScheduledDate}', JobcardNotes = '{$Notes}', TechReport = '{$TechNotes}', JobcardStatus = {$NewStatus}, InvoiceID = {$InvoiceID},ProjectID = {$ProjectID}, ManualJobcardNumber = '{$ManualJobcardNumber}', TotalTime = '{$TotalTime}', WorkOrder = '{$WorkOrder}' WHERE JobcardID = {$JobcardID} ";
        }
    }
...
function UpdateJobCostData($JobcardID,$ProjectID,$JobCost,$CustomerID){
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    //UPDATE `buscrjrzmx_db1`.`jobcards` SET `jobCost` = '100' WHERE `jobcards`.`JobcardID` = 2782;
    $UPDATELine = "UPDATE jobcards 
    SET jobCost = '{$JobCost}'
    WHERE JobcardID = {$JobcardID} && ProjectID = {$ProjectID}";
    $DoUPDATELine = mysqli_query($ClientCon, $UPDATELine);
    ...
    if ($Error == "") {
        ...
        return "There was an error updating JobCost";
    }
}
...
function UpdateCompanySettings($CompanyName, $CompanyReg, $VatRegistered, $VatNumber, $VatRate, $ContactNum, $EmailAddress, $FaxNumber, $RecurringDay, $Address1, $Address2, $City, $Region, $PostCode, $Country, $Bank, $AccountHolder, $AccountNumber, $BranchCode, $AccountType, $TermsAndConditions, $accountemail="")
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    ...
    $accountemail = CleanInput($accountemail);
...
function AddCustomItemProject($ProjectID, $CustomItem, $CustomItemPrice, $Quantity, $DiscountPercent, $CostPrice, $CustomerID = "")
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);

    $CustomItem = CleanInput($CustomItem);
    UpdateProjectStatus(2, $ProjectID);
    $Profit = ($CustomItemPrice - $CostPrice) * $Quantity;
    ...
    $ClientCost = $CostPrice;//$CustomItemPrice;
    ...
    $InsertLine = "INSERT INTO customerprojectinvoicelines (ProjectID, InvoiceID,Description, Quantity, Price, ProductID, ProductCode, LineSubTotal, LineDiscount, LineVAT, LineTotal, BillingType, StockAffect, MeassurementDescription, WarehouseID, Profit) ";
    $InsertLine .= "VALUES ({$ProjectID}, 0,'{$ThisProduct}', {$Quantity}, {$ClientCost}, 0, 'CUSTOM', {$SubTotal}, {$DiscountAmount}, {$Vat}, {$LineTotal}, 'Once-Off', {$StockAffect}, '', 0, {$Profit})";
    $DoInsertLine = mysqli_query($ClientCon, $InsertLine);
...
function AddToProjectInvoiceLine($ProjectID, $ProductID, $Price, $Quantity, $DiscountPercent, $WarehouseID, $CustomerID = "")
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    ...
    $LineTotal = $VatableAmount + $Vat;
    $InsertLine = "INSERT INTO customerprojectinvoicelines (ProjectID,InvoiceID, Description, Quantity, Price, ProductID, ProductCode, LineSubTotal, LineDiscount, LineVAT, LineTotal, BillingType, StockAffect, MeassurementDescription, UnitPrice, UnitPriceCost, Profit, WarehouseID) ";
    $InsertLine .= "VALUES ({$ProjectID}, 0, '{$ThisProduct}', {$Quantity}, {$ClientCost}, {$ProductID}, '{$ProductCode}', {$SubTotal}, {$DiscountAmount}, {$Vat}, {$LineTotal}, '{$BillingType}', {$StockAffect}, '{$MeasurreDescript}', {$UnitCost}, {$SupplierUnitCost}, {$LineProfit}, {$WarehouseID})";
    $DoInsertLine = mysqli_query($ClientCon, $InsertLine);
...
function RemoveCustomItemProjectData($projectInvoiceLineId,$ProjectID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    $DelInvoiceLines = "DELETE FROM customerprojectinvoicelines WHERE ProjectID = {$ProjectID} AND  ProjectInvoiceLineItemID = {$projectInvoiceLineId}";
    $DoDelInvoiceLines= mysqli_query($ClientCon, $DelInvoiceLines);  
    return "OK";
}
function AddProjectInvoiceData($InvoiceID,$ProjectID,$CustomerID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    ...
    $GotLines = mysqli_query($ClientCon, $GetLines);
    $Insertinovice = "INSERT INTO customerprojectinovice (ProjectId,InvoiceId) ";
        $Insertinovice.= "VALUES ({$ProjectID},{$InvoiceID})";
        $DoInsertLine = mysqli_query($ClientCon, $Insertinovice);
    $Error;
    UpdateProjectStatus(2, $ProjectID);
    while ($invoiceLines = mysqli_fetch_array($GotLines)){
        $Description = $invoiceLines['Description'];
        $Quantity = $invoiceLines['Quantity'];
        $Price = $invoiceLines['Price'];
        $ProductID = $invoiceLines['ProductID'];
        $ProductCode = $invoiceLines['ProductCode'];
        $LineSubTotal = $invoiceLines['LineSubTotal'];
        $LineDiscount = $invoiceLines['LineDiscount'];
        $LineVAT = $invoiceLines['LineVAT'];
        $LineTotal = $invoiceLines['LineTotal'];
        $BillingType = $invoiceLines['BillingType'];
        $StockAffect = $invoiceLines['StockAffect'];
        $MeassurementDescription = $invoiceLines['MeassurementDescription'];
        $UnitPrice = $invoiceLines['UnitPrice'];
        $UnitPriceCost = $invoiceLines['UnitPriceCost'];
        $Profit = $invoiceLines['Profit'];
        $WarehouseID = $invoiceLines['WarehouseID'];    
        $InsertLine = "INSERT INTO customerprojectinvoicelines (ProjectID,InvoiceID, Description, Quantity, Price, ProductID, ProductCode, LineSubTotal, LineDiscount, LineVAT, LineTotal, BillingType, StockAffect, MeassurementDescription, UnitPrice, UnitPriceCost, Profit, WarehouseID) ";
        $InsertLine .= "VALUES ({$ProjectID},{$InvoiceID}, '{$Description}', {$Quantity}, {$Price}, {$ProductID}, '{$ProductCode}', {$LineSubTotal}, {$LineDiscount}, {$LineVAT}, {$LineTotal}, '{$BillingType}', {$StockAffect}, '{$MeassurementDescription}', {$UnitPrice}, {$UnitPriceCost}, {$Profit}, {$WarehouseID})";
        $DoInsertLine = mysqli_query($ClientCon, $InsertLine);
        $Error = mysqli_error($ClientCon);
    }   
...
function GetInvoiceGroupsByMultipleInvoiceId($InvoiceID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);

    $GetGroups = "SELECT * FROM customerinvoicegroups WHERE InvoiceID IN({$InvoiceID}) AND InvoiceGroupID IN (SELECT GroupID FROM customerinvoicelines WHERE InvoiceID IN({$InvoiceID}))";
...
//FUNCTION TO REMOVE AN INVOICE FROM PROJECT
function RemoveProjectInvoiceData($ProjectInvoiceId,$InvoiceID,$ProjectID)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    $GetData = "SELECT * FROM customerprojectinovice WHERE ProjectId = {$ProjectID}";
    $GotDatas = mysqli_query($ClientCon, $GetData);
    $FoundData = mysqli_num_rows($GotDatas);

    if ($FoundData > 1) {
        $DelInvoice = "DELETE FROM customerprojectinovice WHERE ProjectInvoiceId = {$ProjectInvoiceId}";
        $DoDelInvoice = mysqli_query($ClientCon, $DelInvoice);
        $DelInvoiceLines = "DELETE FROM customerprojectinvoicelines WHERE ProjectID = {$ProjectID} AND  InvoiceID = {$InvoiceID}";
        $DoDelInvoiceLines= mysqli_query($ClientCon, $DelInvoiceLines);  
        $DoDelInvoice= mysqli_query($ClientCon, $DelInvoice);
        return "OK";
    }else{
        return "Minimum 1 invoice required";
    }
}
...
function AddEmailTemplates($subjectdata,$purpose ,$contentdata)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    $ThisClientID = $_SESSION["ClientID"];

    $subjectdata = CleanInput($subjectdata);

    $createddate = date("Y-m-d H:i:s");

    $ThisUser = $_SESSION["ClientName"];

    $AddEmailTemplate = "INSERT INTO emailtemplates (subject,purpose, content,createddate) ";
    $AddEmailTemplate .= "VALUES ('{$subjectdata}', '{$purpose}','{$contentdata}','{$createddate}')";
    $DoAddEmailTemplate = mysqli_query($ClientCon, $AddEmailTemplate);
    ...
    if ($Error == "") {
        ...
        return "There was an error creating this Email Template, please check your values and try again";
    }
}

function UpdateEmailTemplates($subjectdata, $purpose, $contentdata,$id)
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    $ThisClientID = $_SESSION["ClientID"];
    ...
    $ThisUser = $_SESSION["ClientName"];
    $UpdateEmailTemplate = "UPDATE `emailtemplates` SET `subject` = '{$subjectdata}',`purpose` = '{$purpose}',`content` = '{$contentdata}' WHERE `id` = {$id}";
    $DoUpdateEmailTemplate = mysqli_query($ClientCon, $UpdateEmailTemplate);
...
{
    $ClientCon = mysqli_connect($_SESSION["DBHost"], $_SESSION["DBUser"], $_SESSION["DBPass"], $_SESSION["DBName"]);
    $GetProjects = "SELECT * FROM emailtemplates";
    $GotProjects = mysqli_query($ClientCon, $GetProjects);
    echo mysqli_error($ClientCon);
...
function GetSingletemplate($id)
{
    ...
    $Gettemplate = "SELECT * FROM emailtemplates WHERE id = {$id}";
    $Gottemplate = mysqli_query($ClientCon, $Gettemplate);
