...
if ($ThisClientID > 0 && $ThisUser != "" && $ThisSecret == "E2A_crm_S5gdbh6nnj_usr_9898")
{
	...
	while ($Val = mysqli_fetch_array($GotOutstanding))
	{
		...
		$GetOutstandingCount = "SELECT COUNT(InvoiceID) AS NumOutstanding FROM customerinvoices WHERE InvoiceStatus IN (1,6) AND DueDate <= '{$Today}' AND CustomerID = {$CustomerID}";
		$GotOutstandingCount = mysqli_query($ClientCon, $GetOutstandingCount);
		echo mysqli_error($ClientCon);
		
		while ($Val2 = mysqli_fetch_array($GotOutstandingCount))
		{
			$NumOutstanding = $Val2["NumOutstanding"];	
		}
	...
	$data[] = array('<b>Customer</b>'=>'','<b>Company Name</b>'=>'', '<b>Outstanding Invoices</b>'=>'<b>Total Outstanding Invoices</b>', '<b>Outstanding Invoice Amount</b>'=>'<b>R' . number_format($AllOutstanding,2) . '</b>');
...
function GetNumberOutstandingAmount($CustomerID)
{
	...
	while ($ValOut = mysqli_fetch_array($GotOutstanding))
	{
		$TotalOutstanding = $ValOut["TotalOutstanding"];	
	}
